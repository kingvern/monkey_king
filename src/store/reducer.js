...
export default (state = initState, action) => {
    ...
    //hasFreePower
    if (action.type === 'free_power_success') {
        const newState = JSON.parse(JSON.stringify(state));
        newState.hasFreePower = true;
        return newState;
    }

    if (action.type === 'add_power_input') {
        const newState = JSON.parse(JSON.stringify(state));
        newState.powerInput = parseInt(newState.powerInput) + 1;
        return newState;
    }

    if (action.type === 'rm_power_input') {
        const newState = JSON.parse(JSON.stringify(state));
        newState.powerInput = parseInt(newState.powerInput) - 1;
        return newState;
    }
